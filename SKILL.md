...
1. Web 界面源码随 Skill 一起存放在 `ui/`。以当前 `SKILL.md` 所在目录为 Skill 根目录，界面目录就是其下的 `ui`，不要依赖桌面项目或固定的绝对路径。
2. 如果 `ui/node_modules` 不存在，先在 `ui` 目录运行 `npm ci` 安装依赖；依赖已存在时不要重复安装。
3. 在 `ui` 目录运行 `npm run dev`，等待终端打印实际的本地访问地址。
4. 使用终端打印的实际地址在浏览器中打开，不要自行假定端口固定为 `3000`。
5. 服务只用于本机访问，不要发布到公网，也不要把用户截图上传到第三方服务。
6. 打开成功后，向用户返回可点击的本地地址，并说明本地服务需要保持运行。
7. 如果启动失败，先检查端口、依赖和沙箱权限；需要额外权限时按运行环境的审批流程请求，不要绕过权限限制。
8. 普通 UX 审查请求不自动打开浏览器；只有用户明确要求打开、启动、演示或用界面操作时才启动。
...
- 如果 Skill 内的 `ui` 目录不存在或不完整，不要临时伪造启动成功；说明界面资源不可用，并继续提供对话式 UX 审查能力。
